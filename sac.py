...
class SAC:
    def __init__(self, env, net: SAC_ACTOR, buffer: ExperienceReplay):
        self.env = env
        self.net = net
        self.buffer = buffer
        self.total_r = 0
...
@torch.no_grad()
def test_agent(env, actor_net: SAC_ACTOR):
    total_r = 0
    actor_net.eval()
...
if __name__ == "__main__":
    ...
    actor_net = SAC_ACTOR(OBS_SPACE_SHAPE, N_ACTIONS).to(DEVICE)
